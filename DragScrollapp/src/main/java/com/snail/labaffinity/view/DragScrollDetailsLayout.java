...
public class DragScrollDetailsLayout extends LinearLayout {
    ...
    private OnChangePagerProgressListener mChangePagerProgressListener;
    ...
    private void scroll(MotionEvent event) {
        ...
        if (mCurrentViewIndex == CurrentTargetIndex.UPSTAIRS) {
            ...
            if (getScrollY() < threshold) {
                // 在第一个界面， 松手后还是第一个界面(1->1)
                float scroolPercent = (float) getScrollY() / threshold;
                if (mChangePagerProgressListener != null) {
                    mChangePagerProgressListener.onUpPagerToUp(CurrentTargetIndex.UPSTAIRS, scroolPercent);
                }
            } else {
                // 在第一个界面， 松手后会进入第二个界面(1->2)
                if (mChangePagerProgressListener != null) {
                    mChangePagerProgressListener.onUpPagerToDown();
                }
            }
            ...
            if (pHeight - getScrollY() < threshold) {
                // 第二个界面， 松手后会进入第二个界面(2->2)
                float scroolPercent = (float) (pHeight - getScrollY()) / threshold;
                if (mChangePagerProgressListener != null) {
                    mChangePagerProgressListener.onDownPagerToDown(CurrentTargetIndex.DOWNSTAIRS, scroolPercent);
                }
            } else {
                // 第二个界面， 松手会进入第一个界面(2->1)
                if (mChangePagerProgressListener != null) {
                    mChangePagerProgressListener.onDownPagerToUp();
                }
            }
    ...
    private void onChangePagerFinish(CurrentTargetIndex targetIndex) {
        if (targetIndex == CurrentTargetIndex.UPSTAIRS) {
            if (mChangePagerProgressListener != null) {
                mChangePagerProgressListener.onUpPagerToUp(targetIndex, 1.0f);
            }
        } else {
            mChangePagerProgressListener.onDownPagerToDown(targetIndex, 1.0f);
        }
    }

    public void setmChangePagerProgressListener(OnChangePagerProgressListener changePagerProgressListener) {
        this.mChangePagerProgressListener = changePagerProgressListener;
    }

    /**
     * 上下页面改变时的状态及进度回调
     */
    public interface OnChangePagerProgressListener {
        /**
         * 上页面向上拉
         *
         * @param status   界面状态
         * @param progress 进度
         */
        void onUpPagerToUp(CurrentTargetIndex status, float progress);

        /**
         * 上页面向下拉
         */
        void onUpPagerToDown();

        /**
         * 下页面向上拉
         */
        void onDownPagerToUp();
         ...
         * @param progress 进度
         */
        void onDownPagerToDown(CurrentTargetIndex status, float progress);
    }
